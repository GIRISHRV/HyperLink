...
export class DashboardPage {
  ...
  async getRecentTransfers(): Promise<TransferItem[]> {
    ...
    for (const element of transferElements) {
      ...
      const directionText = await element.locator('[data-testid="transfer-direction"]').textContent() || "";
      const direction = directionText.toLowerCase().includes("sent") ? "sent" : "received";
      const timestamp = await element.locator('[data-testid="transfer-timestamp"]').textContent() || "";
  ...
  async verifyStatsLoaded(): Promise<void> {
    ...
    await expect(this.page.locator('[data-testid="total-data-moved"]')).toBeVisible({ timeout: 10000 });
    await expect(this.page.locator('[data-testid="total-transfers"]')).toBeVisible({ timeout: 10000 });
    
    // Verify stats are not in loading state
    await expect(this.page.locator('[data-testid="stats-skeleton"]')).not.toBeVisible({ timeout: 5000 });
  }
}
