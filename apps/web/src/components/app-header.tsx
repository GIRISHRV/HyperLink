...
export default function AppHeader({ variant = "app", isPeerReady = false, status = "idle", onBackCheck }: AppHeaderProps) {
    ...
    const NetworkBadge = ({ mobile = false }: { mobile?: boolean }) => (
        ...
        >
            <div className={`size-2 rounded-full ${isOnline ? "bg-green-500 animate-pulse" : "bg-red-500"}`} />
    ...
    const PeerBadge = ({ mobile = false }: { mobile?: boolean }) => (
        ...
        >
            <div className="flex h-3 w-3 relative">
                <span className={`absolute inline-flex h-full w-full rounded-none opacity-75 ${isPeerReady ? "animate-ping bg-green-400" : "bg-red-400"}`} />
                <span className={`relative inline-flex rounded-none h-3 w-3 ${isPeerReady ? "bg-green-500" : "bg-red-500"}`} />
            </div>
            <span className="text-xs font-mono text-gray-400">
                {isPeerReady ? "System Ready" : "Initializing..."}
            </span>
            {email && !mobile && (
                <span className="text-xs font-mono text-white ml-2">• {email}</span>
            )}
        </div>
    );
                ...
                <div className="md:hidden fixed inset-0 z-[60] flex">
                    ...
                    <nav className="relative ml-auto w-72 max-w-[80vw] bg-surface border-l border-subtle flex flex-col animate-in slide-in-from-right duration-200">
                        <div className="flex items-center justify-between px-6 py-5 border-b border-subtle">
                            ...
                            <button aria-label="Close menu" onClick={() => setMobileMenuOpen(false)} className="size-10 flex items-center justify-center text-gray-400 hover:text-white">
                        ...
                        {variant === "transfer" ? (
                            <>
                                <div className="p-6 border-b border-subtle flex flex-col gap-4">
                                    ...
                                    {email && <div className="text-xs font-mono text-white opacity-50 break-all">{email}</div>}
                                </div>
